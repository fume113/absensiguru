...
if ($data && isset($data['date'])) {
    ...
    // Buat query SQL untuk mengambil data guru yang belum absen pada tanggal yang dipilih
    $query = "SELECT data_guru.nama, data_guru.nip
    FROM data_guru
    WHERE data_guru.nip NOT IN (
        SELECT DISTINCT nip
        FROM data_absensi
        WHERE tanggal = ?
    )";
