		...
		for ($i = 0; $i < $weekLength; $i++) {
			for ($j = 1; $j <= 7; $j++) {
				...
				if (!empty($this->currentDay) && $this->currentDay <= $this->currentMonthDaysLength) {
					...
					$dateValue = $this->currentYear . '-' . $this->currentMonth . '-' . str_pad($this->currentDay, 2, '0', STR_PAD_LEFT);
					$weekDays .= '<li data-date="' . $dateValue . '">' . $cellValue . '</li>';
					$this->currentDay++;
				} else {
					$weekDays .= '<li></li>'; // Tambahkan elemen kosong untuk hari yang tidak ada
				}
			}
		}
